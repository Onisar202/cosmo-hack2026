...
| `fixtures/success.json` | Полный успешный расчёт (`historical_forecast`): два окна, окно A доминирует над B по правилу доминирования v2 (не хуже по обоим механизмам, устойчиво лучше по механизму 1 — меньший максимальный уровень и меньшая длительность превышения по каждому порогу). |
