...
| `fixtures/success.json` | Полный успешный расчёт (`historical_forecast`): два окна, окно A доминирует над B по правилу доминирования v2 (не хуже по обоим механизмам, устойчиво лучше по механизму 1 — меньший максимальный уровень и меньшая длительность превышения по каждому порогу). `event_state = EVENT_PRESENT` при `status = ok` — форма, где событие подтверждено архивной линией И количественно оценено другой линией того же механизма; `source_agreement = CONSISTENT` с двумя линиями в `source_assessments` — форма подтверждения события двумя независимыми поставщиками. |
...
| `fixtures/source-error.json` | Архив источника космической погоды отвечает 429 (`source_error`, `quota_limited: true`) независимо от отдельного объективного пробела в архиве (`coverage.archive_gaps`); отказ источника не превращается в «фон». `event_state = INSUFFICIENT_DATA` — отказ получения не даёт права сказать «событий не было» (FN-41). |
| `fixtures/equal-windows.json` | Два окна идентичны по обоим механизмам — правило доминирования v2 даёт `recommendation.status = "tie"`, рекомендация не выдаётся. |
...
форму контракта и валидны по `result.schema.json`. FN-41 обновил обе
исторические фикстуры (`success`, `source-error`) на реально реализованный
источник орбиты: `nasa-iss-oem-history` / `EME2000` вместо
прежнего `space-track` (GP_HISTORY по-прежнему задокументирован, но не
реализован) — форма контракта не должна демонстрировать путь, которого в
сервисе нет.
...
- `result` per-mechanism (`mechanismAssessment.status`) различает **семь**
  состояний, а не «есть/нет»:
  `ok | not_implemented | missing_data | stale_data | source_error | beyond_horizon | qualitative_only`.
  ...
  покрыто», а не «спокойно»). `qualitative_only` (FN-41) — наличие
  воздействия ПОДТВЕРЖДЕНО (`event_state = EVENT_PRESENT`), но уровень и
  длительность превышения по доступной линии не восстанавливаются: окно
  выводится из автоматического сравнения (`critical_gap = true`), и это
  «оценить невозможно количественно при подтверждённом событии», а не
  благоприятная оценка и не отсутствие данных. При любом статусе, кроме
  `ok`, `max_level` и `exceedance_hours_by_level` обязаны быть `null` —
  схема запрещает подставлять придуманное значение
  (`if status != "ok" then max_level == null`).
- `mechanismAssessment.event_state` (FN-41, **обязательное поле**) —
  три различимых состояния архивной СОБЫТИЙНОЙ линии Механизма 1 для
  исторических окон плюс `NOT_APPLICABLE`:
  `EVENT_PRESENT | NO_EVENT_DETECTED | INSUFFICIENT_DATA | NOT_APPLICABLE`.
  Это прямое применение main-prompt.md §2 («воздействие не выявлено /
  данные устарели / оценить невозможно — три состояния, не два») к той
  единственной линии космопогоды, у которой есть архив на весь обязательный
  период (уведомления DONKI, `sources.yaml#nasa-donki-notifications`).
  `NO_EVENT_DETECTED` заявляется ТОЛЬКО при подтверждённом покрытии архива
  на нужный интервал — молчание источника само по себе даёт
  `INSUFFICIENT_DATA` с `critical_gap = true` (схема требует это через
  `if`/`then`). `NOT_APPLICABLE` — линия не применялась вовсе: механизм
  `mmod` (схема требует ровно это значение) и `mode = current`, где
  Механизм 1 оценивается наблюдением GOES и внешним прогнозом NOAA.
  Различие обязано быть видно в интерфейсе и в обеих выгрузках, а не только
  в сохранённом объекте.
- `mechanismAssessment.source_agreement` и
  `mechanismAssessment.source_assessments` (FN-41, **оба обязательные**) —
  согласие ИСТОЧНИКОВ ВНУТРИ механизма, отдельно от расхождения МЕХАНИЗМОВ
  между собой (последнее уже выражает правило предпочтения окон,
  `src/domain/windows`; никакого нового ранжирования при этом не вводится).
  `source_agreement` ∈ `CONSISTENT | CONFLICT | INSUFFICIENT_DATA`:
  `CONSISTENT` — не менее двух линий механизма высказались определённо
  (`EVENT_PRESENT`/`NO_EVENT_DETECTED`) и совпали; `CONFLICT` — не менее
  двух высказались определённо и разошлись; `INSUFFICIENT_DATA` —
  определённых высказываний меньше двух, сравнивать нечего (третье
  состояние, а не «источники согласны»). Свежесть/устаревание сюда не
  входят — они отслеживаются отдельно в `source_status`.
  `source_assessments` сохраняет оценку КАЖДОЙ линии как есть (`source_id`,
  свой `event_state`, своя полнота, `beyond_horizon`, свои `record_ids` и
  пояснения) — ни одна линия не выбирается молча и не исчезает при сведении
  к общему состоянию механизма. Схема требует, чтобы `CONFLICT` нёс минимум
  две линии и `critical_gap = true`: расхождение источников выводит окно из
  автоматического сравнения и передаёт его человеку, вместо того чтобы
  ранжировать источники. Пустой `source_assessments` — у механизма нет
  отдельных линий, способных высказаться (например `mmod` с единственным
  источником, или `mode = current`, где наблюдение GOES и суточная
  вероятность NOAA несопоставимы без подобранного порога, который
  main-prompt.md §4 запрещает вводить).
- `result.orbit.source` ∈ `celestrak | space-track | nasa-iss-oem-history`.
  `nasa-iss-oem-history` (FN-41) — датированный выпуск NASA TOPO CCSDS OEM: с этим
  источником и только с ним строятся `historical_analysis`/
  `historical_forecast`. `celestrak` в историческом результате невозможен по
  построению (современные элементы не подставляются вместо исторических,
  main-prompt.md §1, §11); `space-track` (GP_HISTORY) остаётся
  задокументированным, но не реализованным источником.
...
**Правило доминирования v2 (FN-34/S2-04, `src/domain/windows`)** заменило
прежнее лексикографическое правило предпочтения между `mechanismAssessment`
двух окон: окно A доминирует B, только если A не хуже B по каждому
пригодному сопоставимому механизму (ранг `max_level` и покомпонентно
`exceedance_hours_by_level`, при `status == "ok"` у обеих сторон) и
устойчиво лучше хотя бы по одному. Конфликт механизмов (один за A, другой
за B), несопоставимость (готовая оценка есть только у одной стороны) или
неопределённое различие внутри одного механизма (показатели расходятся
разнонаправленно) дают `insufficient_basis`, а не выбор через каскад
приоритетов между `space_weather` и `mmod` — этим правило v2 отличается от
прежнего. `coverage_fraction` в сравнение не входит (полнота данных влияет
только на `critical_gap`, не на «лучше/хуже»). Окно с критическим пробелом
по любому механизму выводится из сравнения (`excluded_from_comparison`),
но остаётся в `result.windows` — не скрывается. Правило требует равной
`duration_hours` у сравниваемых окон; окна разной длительности сравнивать
им запрещено на уровне домена (`WindowComparisonError`).
...
**Каждое предупреждение доказуемо — без исключения по severity, и без
текстовой лазейки.** Три раунда ревью сузили это правило до нынешней формы:
раунд 1 указал, что `record_ids` может быть пустым без альтернативы; раунд 2 —
что требование действовало только для `severity = critical`, и `advisory`-
предупреждение об архивном пробеле (`archive-coverage-gap`) осталось без
доказательства, хотя пробел был обнаружен реальной проверкой покрытия
архива; раунд 3 указал, что добавленный тогда свободный текстовый
`no_evidence_reason` сам стал лазейкой — им можно было прикрыть и настоящий
необоснованный вывод, а не только структурно недоказуемый случай.

Итоговое решение — не текстовое исключение, а устранение самой причины: у
**единственного** состояния, для которого в принципе нет ни записи, ни
попытки получения, — `not_implemented`, — нет и собственного объекта в
`warnings`. Оно уже полностью выражено через `mechanismAssessment.status =
"not_implemented"` на каждое окно и механизм (см. `fixtures/incomplete.json`)
— дублировать этот факт ещё и недоказанным предупреждением было
избыточно, и именно эта избыточность требовала лазейки. Любой предупреждение,
которое всё же попадает в `warnings`, описывает то, что реально произошло
(превышение порога, отказ источника, архивный пробел, нарушение
освещённости) — а значит для него всегда достижимо хотя бы одно из:
...
Оба поля разрешено заполнить одновременно (например запись есть, но она
устарела из-за неудачной попытки обновления) — правило требует «хотя бы
одно», а не «ровно одно», и описание в схеме теперь говорит именно это.
