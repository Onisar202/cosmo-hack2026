...
**Каждое предупреждение доказуемо — без исключения по severity.** Раунд 1
ревью ограничивал это требование `severity = critical`; раунд 2 указал,
что `advisory`-предупреждение об архивном пробеле (`archive-coverage-gap`)
тоже осталось без доказательства, хотя пробел был обнаружен реальной
проверкой покрытия архива — то есть доказательство существовало, просто не
было отражено в контракте. Схема теперь требует ровно одно из трёх для
**любого** предупреждения:
- `record_ids` непуст — есть сохранённая запись;
- `fetch_attempt_id` непуст — есть устойчивый идентификатор залогированной
  (в т.ч. неуспешной) попытки обращения к источнику или проверки архива;
  такая попытка/проверка и её результат сохраняются наравне с успешными
  (`.ai/backend-prompt.md` §5 «Логируется каждое обращение к источнику»);
- `no_evidence_reason` непуст — явное текстовое объяснение, почему у
  предупреждения в принципе не может быть ни записи, ни попытки (например
  `mmod-not-implemented`: обращения к источнику не было и не могло быть,
  пробовать было нечего). Это не молчаливое исключение «advisory не
  обязаны» — исключение обязано быть названо прямо в самом объекте.
