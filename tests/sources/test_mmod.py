...
class TestBuildMmodBackgroundRecords:
    def test_builds_one_record_per_row_with_raw_value_not_ratio(self) -> None:
        ...
        # "1 + factor" напрямую, не через src.domain.mmod.background: слой
        # sources не зависит от domain (main-prompt.md §7 «domain/ не знает
        # про источники, sources/ не знает про физику» — верно и в обратную
        # сторону, тест sources/ не должен требовать domain/, чтобы этот
        # файл был самостоятельно тестируем без остального Механизма 2).
        assert first.value != 1.0 + subset[0].factor_105j
